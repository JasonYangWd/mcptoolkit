...
**Real incident:** Hash collision attacks were documented across web frameworks in 2011-2012 (oCERT advisory), demonstrating practical exploitation of hash table vulnerabilities at scale.
...
**Real incident:** CloudFlare's WAF was brought down by ReDoS in 2019; they subsequently switched to the Rust regex crate (non-backtracking) to prevent catastrophic backtracking.
...
**Real incident:** Quicksort attacks documented in academic research (2003); practical exploits in systems with naive pivot selection.
...
**Real incident:** Slowloris-style attacks exploit parameter parsing complexity; similar patterns in JSON path traversal.
...
**Real incident:** Legitimate-looking request floods bypass naive rate limits; common attack pattern in production systems.
...
Each attack above shows a real incident where hardened parsing failed to prevent DoS. Twitter was compromised by hash tables. CloudFlare was hit by regex engines. Sorting and parameter parsing vulnerabilities affect every framework. The pattern is consistent: attackers exploit the algorithms we trust.
